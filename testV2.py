...
class salim:
    ...
    def extract_infos_from_cv_v2(self, cv_text, return_summary=False, file_name=""):
        ...
        7. Work Experiences: For each experience, return a JSON object with "job_title", "company_name", "responsibilities", "skills", "city", "start_date," and "end_date". in "responsibilities", list tasks as a string, enclosed with "#start#" at the beginning of every task sentence, and the string "#end#" at the end of every task sentence extracted from the resume text. if tasks aren’t explicitly listed, intelligently extract them from the resume text (task sentences shouldn't start with transition words such as "Additionally", "Subsequently", or similar transitional adverbs, they should be complete imformative sentences). In "skills", if there was a paragraph dedicated to listing skills in the experience text (only in the text of said experience, not in other sections), put it here (it's usually preceeded with a title such as "Technical Skills", "Skills", "Technical Environment", or similar titles, written in the language of the resume). if there was no paragraph listing the skills acquired on that experience, then leave "skills" empty. For dates, mark one ongoing role with "present" as "end_date" and set missing dates to empty strings. every "start_date" and "end_date" should contain both the month and year if they exist, or only the year if the month doesn't exist. sort experiences from most recent to oldest.
                            ...
                            try:
                                if key == "work":
                                    ...
                                    for key_work, work in enumerate(temp_work):
                                        ...
                                        temp_work[key_work]["responsibilities"] = f"<p>{self.translate_responsibilities(language)}:</p>{temp_work[key_work]["responsibilities"]}"
